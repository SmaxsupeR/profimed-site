...
export function Prices({
  ...
  // Найденный баг — при открытии сайта страница сама пролистывалась вниз
  // до прайса. Причина: этот эффект уже срабатывает на самом первом
  // рендере (React вызывает useEffect и после монтирования, не только
  // при последующих изменениях category), а комментарий про block:
  // 'nearest' ошибочно предполагал, что это защищает от вертикального
  // скролла всегда — на деле 'nearest' только выбирает МИНИМАЛЬНОЕ
  // расстояние прокрутки, а не отменяет её: если элемент целиком вне
  // текущего viewport (как вкладка «Все» при первой отрисовке — сама
  // секция Прайс ещё далеко за пределами экрана), браузеру всё равно
  // приходится проскроллить страницу, чтобы дотянуться до него.
  //
  // Первая попытка фикса — флаг isFirstCategorySync ref, «пропустить самый
  // первый вызов эффекта» — не сработала в dev: StrictMode (main.jsx)
  // намеренно вызывает каждый эффект дважды при монтировании (mount →
  // cleanup → mount ещё раз), а ref переживает оба вызова одного и того же
  // инстанса компонента — флаг гасит только первый из двух вызовов, второй
  // проходит мимо охраны и всё равно скроллит. Сравнение по значению вместо
  // флага «уже случалось» устойчиво к любому числу повторных вызовов
  // эффекта: скролл происходит только когда category реально отличается от
  // последнего синхронизированного значения — на монтировании (в т.ч.
  // повторном под StrictMode) prevCategoryRef уже равен текущему category
  // (тот же initial-state), сравнение не проходит при любом количестве
  // вызовов; на настоящий клик пользователя по вкладке category меняется —
  // сравнение проходит, происходит и скролл, и обновление ref.
